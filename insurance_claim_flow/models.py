...
class ClaimDecision(BaseModel):
    ...
    deductible: float = Field(
        ..., description="The deductible amount to be paid by the claimant."
    )
    recommended_payout: float = Field(
        ..., description="The final amount recommended for payout."
    )
    notes: Optional[str] = Field(
        None, description="Additional notes regarding the decision."
    )
