...
from .utils import parse_synthea_patient
...
class GuidelineRecommendationWorkflow(Workflow):
    ...
    async def handle_guideline_match(
        ...
        MAX_PATIENT_INFO_TOKENS = 2000
        MAX_CONDITION_INFO_TOKENS = 2000

        def truncate_text(text, max_tokens=2000):
            """Truncate text to a maximum number of characters (approximate token count)."""
            return text[:max_tokens]

        guideline_queries = await self.llm.astructured_predict(
            GuidelineQueries,
            prompt,
            patient_info_str = truncate_text(patient_info.demographic_str, MAX_PATIENT_INFO_TOKENS),
            condition_info_str = truncate_text(ev.bundle.json(), MAX_CONDITION_INFO_TOKENS)
        ...
        MAX_GUIDELINE_TOKENS = 2000  # Adjust based on the OpenAI token limit
        def truncate_text(text, max_tokens=MAX_GUIDELINE_TOKENS):
            return text[:max_tokens]  # Simple truncation (alternative: use tiktoken for accurate token counting)
        guideline_text = "\n\n".join([truncate_text(g.get_content(), MAX_GUIDELINE_TOKENS) for g in guideline_docs])
        # guideline_text="\n\n".join([g.get_content() for g in guideline_docs])
        if self._verbose:
            ctx.write_event_to_stream(
                LogEvent(msg=f">> Found guidelines: {guideline_text[:400]}...")
            )
